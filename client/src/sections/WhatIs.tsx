    ...
    <section id="whatis" className="py-24 px-6" data-testid="section-whatis">
      <div className="max-w-[1100px] mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-[#5a5a72] text-lg max-w-[640px] mx-auto">{t('whatis.desc')}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <div
              key={i}
              className="fi bg-white border border-[rgba(124,92,252,0.1)] rounded-2xl p-8 hover:border-[rgba(124,92,252,0.3)] transition-all group"
            >
              ...
              <p className="text-[#5a5a72] text-[15px] leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
