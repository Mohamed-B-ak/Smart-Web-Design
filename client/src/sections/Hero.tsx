  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-32 pb-16 overflow-hidden" data-testid="section-hero">
      ...
      <div
        ...
        style={{
          backgroundImage: 'linear-gradient(rgba(90,24,154,.04) 1px, transparent 1px), linear-gradient(90deg, rgba(90,24,154,.04) 1px, transparent 1px)',
      ...
      <div className="relative z-10 max-w-[820px] mx-auto">
        ...
        <div className="flex items-center justify-center gap-3.5 mb-12 flex-wrap animate-fade-up animation-delay-300">
          <a href="#demo" className="group inline-flex items-center gap-2 px-8 py-3.5 text-[15px] font-semibold text-white gradient-bg glow rounded-full hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(90,24,154,0.4)] transition-all duration-300 shimmer" data-testid="button-hero-try">
            <Phone className="w-4 h-4 group-hover:rotate-12 transition-transform" />
            {t('hero.cta_try')}
          </a>
          <a href="#" className="group inline-flex items-center gap-2 px-8 py-3.5 text-[15px] font-semibold text-[#1a0a2e] border border-[rgba(90,24,154,0.15)] rounded-full hover:bg-[rgba(90,24,154,0.05)] hover:border-[rgba(90,24,154,0.3)] transition-all duration-300 backdrop-blur-sm" data-testid="button-hero-book">
            {t('hero.cta_book')}
      ...
      <div className="relative z-10 max-w-[1060px] w-full mx-auto rounded-3xl overflow-hidden border border-[rgba(90,24,154,0.15)] bg-[rgba(255,255,255,0.85)] backdrop-blur-xl shadow-[0_0_100px_rgba(90,24,154,0.15),0_40px_80px_rgba(0,0,0,0.08)] animate-fade-up animation-delay-500 ai-glow">
        <div className="p-6 md:p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-[12px] text-[#8878a0] ml-2 font-mono">sondos-ai-dashboard</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[rgba(90,24,154,0.06)] border border-[rgba(90,24,154,0.1)] rounded-2xl p-5 hover:bg-[rgba(90,24,154,0.1)] transition-colors duration-300">
              <p className="text-[12px] text-[#8878a0] uppercase tracking-wider mb-1">{lang === 'en' ? 'Active Calls' : 'مكالمات نشطة'}</p>
              <p className="text-3xl font-bold text-[#1a0a2e]">1,247</p>
              <p className="text-[13px] text-[#00d68f] mt-1">+12.5%</p>
            </div>
            <div className="bg-[rgba(90,24,154,0.06)] border border-[rgba(90,24,154,0.1)] rounded-2xl p-5 hover:bg-[rgba(90,24,154,0.1)] transition-colors duration-300">
              <p className="text-[12px] text-[#8878a0] uppercase tracking-wider mb-1">{lang === 'en' ? 'Resolution Rate' : 'نسبة الحل'}</p>
              <p className="text-3xl font-bold text-[#1a0a2e]">94.8%</p>
              <p className="text-[13px] text-[#00d68f] mt-1">+3.2%</p>
            </div>
            <div className="bg-[rgba(90,24,154,0.06)] border border-[rgba(90,24,154,0.1)] rounded-2xl p-5 hover:bg-[rgba(90,24,154,0.1)] transition-colors duration-300">
              <p className="text-[12px] text-[#8878a0] uppercase tracking-wider mb-1">{lang === 'en' ? 'Avg Response' : 'متوسط الاستجابة'}</p>
      ...
      <div className="relative z-10 max-w-[1060px] w-full mx-auto mt-12 animate-fade-up animation-delay-600">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-0">
          ...
          <div className="flex-1 text-center">
            <p className="text-4xl sm:text-5xl font-bold gradient-text">{t('hero.stat2_val')}</p>
            <p className="text-sm text-[#4a3a62] mt-2">{t('hero.stat2_label')}</p>
          </div>
          <div className="hidden sm:block w-px h-16 bg-gradient-to-b from-transparent via-[rgba(90,24,154,0.2)] to-transparent" />
          <div className="flex-1 text-center">
            <p className="text-4xl sm:text-5xl font-bold gradient-text">{t('hero.stat3_val')}</p>
            <p className="text-sm text-[#4a3a62] mt-2">{t('hero.stat3_label')}</p>
          </div>
          <div className="hidden sm:block w-px h-16 bg-gradient-to-b from-transparent via-[rgba(90,24,154,0.2)] to-transparent" />
          <div className="flex-1 text-center sm:text-right">
            <p className="text-4xl sm:text-5xl font-bold gradient-text">{t('hero.stat4_val')}</p>
            <p className="text-sm text-[#4a3a62] mt-2">{t('hero.stat4_label')}</p>
          </div>
        </div>
      </div>
    </section>
