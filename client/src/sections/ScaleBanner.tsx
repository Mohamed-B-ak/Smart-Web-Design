  ...
  return (
    <section className="py-20 px-6" data-testid="section-scale-banner">
      <div className="max-w-[1100px] mx-auto text-center">
        ...
        <p className="text-[#5a5a72] text-lg mb-14">{t('scale.subtitle')}</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <div
              key={i}
              className="fi bg-white border border-[rgba(124,92,252,0.12)] rounded-2xl p-8"
            >
              <p className="text-4xl md:text-5xl font-bold gradient-text mb-2">{stat.value}</p>
              <p className="text-[14px] text-[#8a8aa0]">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
