  ...
  return (
    <section className="py-24 px-6" data-testid="section-integrations">
      <div className="max-w-[1100px] mx-auto text-center">
        ...
        <p className="text-[#a1a1b5] text-lg max-w-[580px] mx-auto mb-14">{t('integrations.desc')}</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {integrations.map((item, i) => (
            <div
              key={i}
              className="fi flex items-center gap-3 bg-[#12121c] border border-[rgba(124,92,252,0.08)] rounded-xl p-4 hover:border-[rgba(124,92,252,0.25)] transition-all"
            >
              <div
              ...
              <span className="text-[14px] font-medium text-[#d1d1d8] truncate">{item.name}</span>
            </div>
          ))}
        </div>
      </div>
